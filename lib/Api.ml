...
let current_config = {
    port = ref 8732;
    basedir = ref "/Users/user/.tezos-client";
...
let make_context () =
  let rpc_config : RPC_client_unix.config = {
      ...
      endpoint = Uri.of_string "https://granadanet.api.tez.ie";
...
let exception_handler =
  ...
  | exn ->
     Lwt.return_error [error_of_exn exn] )
...
let check_mempool oph =
  Alpha_block_services.Mempool.pending_operations
       ...
       ()
  >>= function
  | Ok {applied; unprocessed; refused; branch_refused; branch_delayed; _} ->
     begin
